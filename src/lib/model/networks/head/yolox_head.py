...
class YOLOXHead(nn.Module):
    ...
    def forward(self, inputs):
        ...
        if self.use_checkpoint:
            ...
            # class branch
            cls_feat = checkpoint(self.cls_convs_wrapper, inputs,
                                  self.dummy_tensor)
            cls_output = checkpoint(self.cls_preds_wrapper, cls_feat,
                                    self.dummy_tensor)
            ...
            # reg correlation
            reg_output = self.correction_depth(reg_output, cls_output)

        # merge output
        if self.acts[-2] == "":  # 需要手动进行simgoid
            outputs = torch.cat([reg_output, cls_output.sigmoid()],
                                1)  # B, 2, h, w
        else:
