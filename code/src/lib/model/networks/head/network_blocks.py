...
class BaseConv(nn.Module):
    """A Conv2d -> LayerNorm([C, H, W]) -> activation block."""
    ...
    def __init__(
        ...
        if height is not None and width is not None:
            self.ln = nn.LayerNorm([out_channels, height, width])
        else:
            num_groups = max(1, out_channels // 32)
            self.ln = nn.GroupNorm(num_groups, out_channels)
